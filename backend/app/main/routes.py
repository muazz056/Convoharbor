...
@main.route('/helicone/status', methods=['GET'])
@swag_from({
    'tags': ['Observability'],
    'summary': 'Get Helicone integration status',
    'description': 'Returns the current status of Helicone observability integration',
    'responses': {
        '200': {
            'description': 'Helicone status retrieved successfully',
            'schema': {
                'type': 'object',
                'properties': {
                    'enabled': {'type': 'boolean'},
                    'auth_configured': {'type': 'boolean'},
                    'openai_proxied': {'type': 'boolean'},
...
def helicone_status():
    """Get Helicone observability integration status"""
    try:
        if current_app.llm_service:
            helicone_info = current_app.llm_service.get_helicone_status()
        else:
            helicone_info = {
                "enabled": False,
                "auth_configured": False,
                "openai_proxied": False,
                "proxy_url": None,
                "error": "LLM service not available"
            }
        
        # Add additional information
        helicone_info.update({
            "auth_env_var": "HELICONE_AUTH or HELI_CONE_AUTH",
            "integration_method": "proxy",
            "supported_providers": ["openai"],
            "documentation": "https://docs.helicone.ai/getting-started/integration-method/openai-proxy"
        })
        
        current_app.logger.info(f"📊 Helicone status check: enabled={helicone_info.get('enabled', False)}")
        
        return jsonify(helicone_info), 200
        
    except Exception as e:
        current_app.logger.error(f"❌ Helicone status check failed: {e}")
        return jsonify({
            "error": "Failed to get Helicone status",
            "message": str(e),
            "enabled": False
        }), 500