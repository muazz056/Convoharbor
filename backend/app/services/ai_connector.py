...
class AIConnectorService:
    ...
    def query_knowledge_base(self, query: str, session_id: Optional[str] = None,
        ...
        try:
            ...
            if chatbot_config:
                if 'ai_model' in chatbot_config:
                    # Map chatbot model to AI service parameters
                    model_config = self._map_chatbot_model(chatbot_config['ai_model'])
    ...
    def _map_chatbot_model(self, chatbot_model: str) -> Dict[str, Any]:
        """
        Map chatbot model configuration to AI service parameters
        
        Args:
            chatbot_model: Chatbot's configured AI model
            
        Returns:
            Mapped configuration for AI service
        """
        # Model mapping for different AI providers
        model_mapping = {
            # OpenAI models
            'gpt-4o': {'llm_provider': 'openai', 'model': 'gpt-4o'},
            'gpt-4o-mini': {'llm_provider': 'openai', 'model': 'gpt-4o-mini'},
            'gpt-4-turbo': {'llm_provider': 'openai', 'model': 'gpt-4-turbo'},
            'gpt-3.5-turbo': {'llm_provider': 'openai', 'model': 'gpt-3.5-turbo'},
            
            # Gemini models
            'gemini-1.5-pro': {'llm_provider': 'gemini', 'model': 'gemini-1.5-pro'},
            'gemini-1.5-flash': {'llm_provider': 'gemini', 'model': 'gemini-1.5-flash-latest'},
            'gemini-pro': {'llm_provider': 'gemini', 'model': 'gemini-pro'},
        }
        
        return model_mapping.get(chatbot_model, {
            'llm_provider': 'openai', 
