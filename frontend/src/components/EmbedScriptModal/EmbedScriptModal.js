...
const EmbedScriptModal = ({ isOpen, onClose, script, chatbotId, publicAppUrl }) => {
  ...
  const aiPrompts = {
    ...
    next: "Create a new file components/ConvharborWidget.tsx with the code below, then import <ConvharborWidget /> in your app/layout.tsx inside <body>. Set NEXT_PUBLIC_CHATBOT_ID and NEXT_PUBLIC_PUBLIC_APP_URL in your .env.local file.",
    vue: "Create plugins/convoharbor.ts (Nuxt) or copy the code into main.ts (Vue CLI/Vite). Set VITE_CHATBOT_ID and VITE_PUBLIC_APP_URL in your .env file.",
